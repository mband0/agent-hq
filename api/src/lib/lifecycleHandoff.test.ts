...
describe('markTaskNeedsAttentionForMissingSemanticHandoff', () => {
  it('writes a structured operator recovery note for missing lifecycle handoff', () => {
    ...
    expect(note?.content).toContain('Tests run: runtime completion reconciliation and lifecycle handoff enforcement');
    ...
    expect(note?.content).toContain('Evidence: instance_id=2028; session_key=run:2028; lane=qa; prior_status=review; runtime_success=yes; review_qa_deploy_evidence_recorded=no; runtime_end_source=instance_complete; runtime_ended_at=2026-05-01T19:44:23.584Z');
    expect(note?.content).toContain('Next action: operator review before any redispatch or lane re-entry');
    expect(note?.content).toContain('Next owner: PM/operator');
    expect(note?.content).toContain('Instance ID: 2028');
    expect(note?.content).toContain('Session key: run:2028');
    expect(note?.content).toContain('Lane: qa');
    expect(note?.content).toContain('Prior task status: review');
    expect(note?.content).toContain('Runtime ended successfully: yes');
    expect(note?.content).toContain('Review/QA/deploy evidence recorded: no');
  });

  it('preserves runtime-success context in the operator note for quarantined outcome-less completions', () => {
    const db = new Database(':memory:');
    db.exec(`
      CREATE TABLE tasks (
        id INTEGER PRIMARY KEY,
    ...
    db.prepare(`INSERT INTO tasks (id, title, status) VALUES (404, 'Task 404', 'review')`).run();
    db.prepare(`INSERT INTO job_instances (id, task_id, runtime_ended_at) VALUES (2029, 404, '2026-05-01T21:25:17.102Z')`).run();
    ...
    const note = db.prepare(`SELECT content FROM task_notes WHERE task_id = 404`).get() as { content: string } | undefined;
    expect(note?.content).toContain('Failure or issue observed: runtime ended successfully at the session level without the required lifecycle handoff');
    expect(note?.content).toContain('Evidence: instance_id=2029; session_key=run:2029; lane=review; prior_status=review; runtime_success=yes; review_qa_deploy_evidence_recorded=yes; runtime_end_source=instance_complete; runtime_ended_at=2026-05-01T21:25:17.102Z');
    expect(note?.content).toContain('Lane: review');
    expect(note?.content).toContain('Runtime ended successfully: yes');
