...
registerTool(
  ...
  'Create a new agent.',
  {
    ...
    repo_path: z.string().nullable().optional().describe('Canonical local git repo path for worktree mode'),
    repo_url: z.string().nullable().optional().describe('Remote git URL for clone mode'),
    repo_access_mode: z.enum(['worktree', 'clone']).nullable().optional().describe('Explicit repo source mode'),
...
registerTool(
  ...
  'Create a story-point model-routing rule.',
  {
    ...
    thinking_level: z.enum(['off', 'minimal', 'low', 'medium', 'high', 'adaptive']).nullable().optional().describe('Optional reasoning effort override'),
